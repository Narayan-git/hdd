...
<body>
    ...
    <form class="text-center">
        ...
        <input type="text" class="mb-3" id="pName" onkeyup="myFunction()" placeholder="Search for names.." title="Type in a name">
        ...
        <thead class="thead-light">
            ...
            <?php
                ...
                while($data = mysqli_fetch_assoc($result)){
                    ...
                    if($data['status']==0)
                        $draw.= "<td><a href = 'uUpdateValue.php ? tr_id=".$data['tr_id']."& test_id=".$data['test_id']."& pid=".$data['pid']."& name=".$name['name']."' class='btn btn-danger'>PENDING</a></td>";
                    else 
                        $draw.= "<td><a href = 'GenReport.php ? tr_id=".$data['tr_id']."& test_id=".$data['test_id']."& pid=".$data['pid']."& name=".$name['name']."' class='btn btn-success'>COMPLETED</a></td>";

                    if($data['status']==0)
                        $draw.="<td><button type='button' class='btn btn-danger'  disabled>GET REPORT</button> </td></tr>";
                    else
                        $draw.="<td><a href = 'GenReport.php ? tr_id=".$data['tr_id']."& test_id=".$data['test_id']."& pid=".$data['pid']."& name=".$name['name']."' class='btn btn-success'>GET REPORT </a></td></tr>";
                    $count+=1;
                }
                echo $draw;
            ?>
